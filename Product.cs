...
namespace POSProject
{
    ...
    public partial class Product : Form
    {
        ...
        public void LoadProduct()
        {
            ...
            cm = new SqlCommand("SELECT p.pcode, p.barcode, p.pdesc, b.bid, c.cid, p.price, p.reorder FROM tbProduct As p INNER JOIN tbBrand AS b ON b.id = p.bid INNER JOIN tbCategory AS c ON c.id = p.cid WHERE CONCAT(p.pdesc, b.brand, c.category) LIKE '%" + txtSearch.Text + "%'", cn);
            cn.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void dgvProduct_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (colName == "Delete")
            {
                if (MessageBox.Show("Are you sure you want to delete this record?", "Delete Record", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    cm.ExecuteNonQuery();
                    cn.Close();
                    MessageBox.Show("Product has been successfully deleted", "POS", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                }
            }
            LoadProduct();
        }
