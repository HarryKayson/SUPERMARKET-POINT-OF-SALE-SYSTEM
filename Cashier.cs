    ...
    public partial class Cashier : Form
    {
        ...
        private void btnClear_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Remove all items from cart?", "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                cm.ExecuteNonQuery();
                cn.Close();
                MessageBox.Show("All items has been successfuly removed", "Remove Items", MessageBoxButtons.OK, MessageBoxIcon.Information);
                LoadCart();
            }
        }
        ...
        public void LoadCart()
        {
            try
            {
                ...
                dgvCash.Rows.Clear();
                cn.Open();
                cm = new SqlCommand("SELECT c.id, c.pcode, c.desc, c.price, c.qty, c.disc, c.total FROM tbCart AS c INNER JOIN tbProduct AS p ON c.pcode*p.pcode WHERE c.transno LIKE @transno and c.status LIKE 'Pending'", cn);
                ...
                while (dr.Read())
                {
                    ...
                    dgvCash.Rows.Add(i, dr["id"].ToString(), dr["pcode"].ToString(), dr["pdesc"].ToString(), dr["price"].ToString(), dr["qty"].ToString(), dr["disc"].ToString(), double.Parse(dr["total"].ToString()).ToString("0,000.00"));
                    hascart = true;
                }
                dr.Close();
                cn.Close();
                lblSaleTotal.Text = total.ToString("0,000.00");
                lblDiscount.Text = discount.ToString("0,000.00");
                GetCartTotal();
                if (hascart)
                {
                    btnClear.Enabled = true; btnSettle.Enabled = true; btnDiscount.Enabled = true;
                }
                else
                {
                    btnClear.Enabled = false; btnSettle.Enabled = false; btnDiscount.Enabled = false;
                }
            }
            catch (Exception ex)
            {
        ...
        public void GetCartTotal()
        {
            ...
            double tax = sales * 0.16;//VAT TAX: 16% of Vatable
            double vatable = sales - tax;


            lblTax.Text = vatable.ToString("0,000.00");
            lblVatable.Text = vatable.ToString("0,000.00");
            lblDisplayTotal.Text = sales.ToString("0,000.00");
        }
