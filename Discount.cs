    ...
    public partial class Discount : Form
    {
        ...
        private void txtDiscount_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                txtDiscAmount.Text = disc.ToString("0,000.00");
            }
            catch(Exception)
            {
        ...
        private void btnConfirm_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    cn.Open();
                    cm = new SqlCommand("UPDATE tbCart SET disc_percentage=@disc_percentage WHERE id = @id", cn);
                    cm.Parameters.AddWithValue("@disc_percentage", double.Parse(txtDiscount.Text));
